...
{this._rerenderCalendar();$(this.ele).trigger('dpMonthChanged',[this.displayedMonth,this.displayedYear]);}},setSelectedRange:function(start,end,moveToMonth,dispatchEvents){if(this.selectMultiple==false){return;}
...
var selectorString='td.'+(start.getMonth()==this.displayedMonth?'current-month':'other-month');var cellDate=null;$(selectorString,this.context).each(function(){cellDate=new Date($(this).data('datePickerDate'));if(cellDate.valueOf()>=startValue&&cellDate.valueOf()<=endValue){$(this).addClass('selected');}});if(dispatchEvents){}},setSelected:function(d,v,moveToMonth,dispatchEvents)
...
{$e=$(this.ele);var dClone=new Date(s.valueOf());$e.trigger('dateSelected',[dClone,$(this),false]);$e.trigger('change');}}
